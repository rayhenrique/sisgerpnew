...
function makeExpensesTable(opts: {
  ...
  const builder = {
    ...
    single: vi.fn(async () => {
      if (state.operation === "insert") {
        return opts.insertResult ?? { data: { id: "exp-1" }, error: null };
      }
      return opts.updateResult ?? { data: { id: "exp-1" }, error: null };
    }),
...
describe("expenses api", () => {
  ...
  it("envia payload correto ao criar despesa", async () => {
    const expensesTable = makeExpensesTable({});
    ...
    const supabase = {
      ...
      from: vi.fn((table: string) => {
        if (table === "expenses") return expensesTable;
    ...
    expect(expensesTable.insert).toHaveBeenCalledTimes(1);
    const payload = (expensesTable.__state.insertPayload ?? {}) as Record<string, unknown>;
    expect(payload.description).toBe("despesa teste");
    ...
    expect(payload.classification_id).toBe("class-1");
    ...
    expect(payload.expense_classification_id).toBe("class-1");
  });
  ...
  it("valida entrada antes de chamar o banco", async () => {
    getSupabaseBrowserClientMock.mockReturnValueOnce({
      from: vi.fn(),
      auth: { getUser: vi.fn() },
    });
  ...
  it("faz update consultando linha anterior e salva audit log", async () => {
    const expensesTable = makeExpensesTable({
      oldRowResult: { data: { id: "exp-1", description: "antiga" }, error: null },
      updateResult: { data: { id: "exp-1", description: "nova" }, error: null },
    });
    const auditLogsTable = makeAuditLogsTable();
    ...
    expect(expensesTable.maybeSingle).toHaveBeenCalledTimes(1);
    expect(expensesTable.update).toHaveBeenCalledTimes(1);
